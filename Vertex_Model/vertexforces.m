function [Fs, F_elastic, F_tension, F_contractility]=vertexforces(locs,...
    ...
    for ix = 1:length(cells)
        ...
        for i = 1:length(ixs)
            F_elastic(ixs(i),:)=F_elastic(ixs(i),:)+0.5.*(cell_area-A0).*...
    ...
    F_tension=lambda.*F_tension;
    F_contractility=gamma.*F_contractility;
